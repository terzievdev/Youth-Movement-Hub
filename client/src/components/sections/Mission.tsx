    ...
    <section id="mission" className="py-16 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-12 md:mb-20">
          <h2 className="text-3xl md:text-6xl font-serif font-bold text-primary mb-4 md:mb-6 uppercase tracking-tight">Защо сме тук?</h2>
          <div className="w-20 md:w-32 h-1 bg-accent mx-auto" />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center max-w-6xl mx-auto">
          ...
          <div className="flex flex-col gap-6 md:gap-10 w-full">
            {cards.map((card, index) => (
              <div 
                key={index}
                className="perspective-1000 h-[280px] md:h-[300px] cursor-pointer group w-full"
                onClick={() => isMobile && setFlipped(flipped === index ? null : index)}
              >
                <motion.div
                  initial={false}
                  animate={{ rotateY: isMobile ? (flipped === index ? 180 : 0) : 0 }}
                  whileHover={!isMobile ? { rotateY: 180 } : undefined}
                  transition={{ duration: 0.6, ease: "easeInOut" }}
                  className="relative w-full h-full preserve-3d"
                >
                  {/* Front */}
                  <div className="absolute inset-0 backface-hidden bg-white/10 backdrop-blur-[10px] rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-0 overflow-hidden border border-white/20 flex flex-col md:flex-row">
                    <div className="h-28 md:h-full md:w-1/3 relative">
                      <img src={card.image} className="w-full h-full object-cover" alt={card.title} />
                      <div className="absolute inset-0 bg-gradient-to-b md:bg-gradient-to-r from-transparent via-white/5 to-white/20" />
                    </div>
                    <div className="flex-1 p-5 md:p-8 flex flex-col justify-center">
                      <h3 className="text-2xl md:text-3xl font-serif font-bold text-primary mb-1">{card.title}</h3>
                      <p className="text-primary leading-relaxed text-sm md:text-[16px] font-bold italic">{card.frontText}</p>
                      <p className="text-[#D4AF37] text-xs font-serif mt-2 tracking-widest uppercase md:hidden">Натисни за повече →</p>
                    </div>
                  </div>

                  {/* Back */}
                  <div 
                    className="absolute inset-0 backface-hidden bg-[#fdfbf7] backdrop-blur-[25px] rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-6 md:p-10 flex flex-col items-center justify-center text-center text-primary border border-[#e5d5c5]/50 overflow-hidden"
                  ...
                  >
                    ...
                    <p className={`leading-relaxed tracking-wide text-primary font-serif italic px-2 md:px-6 font-bold relative z-10 ${card.title === "Мисия" ? "text-[13px] md:text-[16px]" : "text-[14px] md:text-[18px]"}`} style={{ textShadow: "0 0 10px rgba(253, 251, 247, 1), 0 0 20px rgba(253, 251, 247, 0.8), 1px 1px 2px rgba(0,0,0,0.1)" }}>
                      {card.back}
                    </p>
                  </div>
                </motion.div>
